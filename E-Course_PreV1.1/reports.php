...
if(isset($_POST['remove'])){
   ...
   if($user_id != ''){
      ...
      $verify_likes = $conn->prepare("SELECT * FROM `report` WHERE user_id = ? AND content_id = ?");
      $verify_likes->execute([$user_id, $content_id]);

      if($verify_likes->rowCount() > 0){
         $remove_likes = $conn->prepare("DELETE FROM `report` WHERE user_id = ? AND content_id = ?");
...
<section class="liked-videos">
   ...
   <h1 class="heading">Reported videos</h1>

   <div class="box-container">

   <?php
      $select_likes = $conn->prepare("SELECT * FROM `report` WHERE user_id = ?");
   ...
   <div class="box">
      ...
      <form action="" method="post" class="flex-btn">
         ...
         <a href="watch_video.php?get_id=<?= $fetch_contents['id']; ?>" class="inline-btn">watch</a>
         <input type="submit" value="remove" class="inline-delete-btn" name="remove"><!-- comment -->
      </form>
   </div>
   <?php
            }
      ...
      echo '<p class="empty">nothing added to report yet!</p>';
   }
   ?>
