...
if(isset($_POST['report_content'])){
   ...
   if($user_id != ''){
      ...
      $select_report = $conn->prepare("SELECT * FROM `report` WHERE user_id = ? AND content_id = ?");
      $select_report->execute([$user_id, $content_id]);

      if($select_report->rowCount() > 0){
         $remove_report = $conn->prepare("DELETE FROM `report` WHERE user_id = ? AND content_id = ?");
         $remove_report->execute([$user_id, $content_id]);
         $message[] = 'removed from report!';
      }else{
         $insert_report = $conn->prepare("INSERT INTO `report`(user_id, tutor_id, content_id) VALUES(?,?,?)");
         $insert_report->execute([$user_id, $tutor_id, $content_id]);
...
<section class="watch-video">
      ...
      if($select_content->rowCount() > 0){
         while($fetch_content = $select_content->fetch(PDO::FETCH_ASSOC)){
            ...
            $verify_likes->execute([$user_id, $content_id]);
               ...
               $select_report = $conn->prepare("SELECT * FROM `report` WHERE content_id = ?");
            $select_report->execute([$content_id]);
            $total_report = $select_report->rowCount();  

            $verify_report = $conn->prepare("SELECT * FROM `report` WHERE user_id = ? AND content_id = ?");
            $verify_report->execute([$user_id, $content_id]);
   ...
   <div class="video-details">
      ...
      <h1 class="level"><?= $fetch_content['level']; ?></h1>
      <div class="info">
         <p><i class="fas fa-calendar"></i><span><?= $fetch_content['date']; ?></span></p>
         <p><i class="fas fa-heart"></i><span><?= $total_likes; ?> likes</span></p>
           <p><i class="fas fa-flag"></i><span><?= $total_report; ?> Reports</span></p>
      </div>
      <div class="tutor">
      ...
      <form action="" method="post" class="flex">
         ...
         <?php
            if($verify_report->rowCount() > 0){
         ?>
        
         <button type="submit" name="report_content"><i class="fas fa-flag"></i><span>Reported</span></button>
           <br>
         <?php
         }else{
         ?>
     
         <button type="submit" name="report_content"><i class="far fa-flag"></i><span>Report</span></button>
           <br>
         <?php
            }
         ?>
...
<section class="comments">
   ...
   <h1 class="heading">Educator comments</h1>

   
   <div class="show-comments">
      <?php
         $select_comments = $conn->prepare("SELECT * FROM `ecomments` WHERE content_id = ?");
         $select_comments->execute([$get_id]);
         if($select_comments->rowCount() > 0){
            while($fetch_comment = $select_comments->fetch(PDO::FETCH_ASSOC)){   
